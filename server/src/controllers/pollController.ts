...
export const createPoll = (req: Request, res: Response, next: NextFunction): void => {
  void (async () => {
...
export const validatePollRequest = (req: Request, res: Response, next: NextFunction): void => {
...
export const linkOpened = (req: Request, res: Response, next: NextFunction): void => {
  void (async () => {
    try {
      // get relevent poll information
      const poll_url_slug = req.params.id;
      const pollInfo = await db.query(SELECT_POLL_BY_URL, [poll_url_slug]);
      ...
      const pollItemNames: string[] = pollItemsInfo.rows.map((column: PollItem): string => {
        console.log('typescript and eslint dont get along');
        return column.name;
      });
